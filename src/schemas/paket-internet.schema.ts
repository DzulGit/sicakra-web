...
export const simpanPaketInternetSchema = z.object({
  ...
  kecepatan_mbps: z.coerce.number().int('Kecepatan harus berupa angka bulat').min(1, 'Kecepatan minimal 1 Mbps'),
  harga: z.coerce.number().min(0, 'Harga tidak boleh negatif'),
  jumlah_perangkat: z.coerce.number().int('Jumlah perangkat harus berupa angka bulat').min(1, 'Minimal 1 perangkat').max(255, 'Maksimal 255 perangkat').optional().default(5),
