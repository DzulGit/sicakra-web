...
import ExportDialog from '@/modules/pendapatan/components/ExportDialog.vue'
import {
  getPelangganResellerList,
  getLaporanPendapatanResellerPdf,
  getLaporanPendapatanResellerExcel,
} from '../api/pendapatanReseller.api'
import { usePendapatanReseller } from '../composables/usePendapatanReseller'

const exportDialogOpen = ref(false)

const { data, isLoading } = usePendapatanReseller()
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-muted-foreground">
          Ringkasan pendapatan dari pelanggan Anda
        </p>
      </div>
    ...
    <DashboardKeuanganTable
      ...
      link-to="/reseller/tagihan"
    />

    <!-- Export Dialog (di-scope ke pelanggan reseller) -->
    <ExportDialog
      v-model:open="exportDialogOpen"
      :fetch-pelanggan="getPelangganResellerList"
      :fetch-pdf="getLaporanPendapatanResellerPdf"
      :fetch-excel="getLaporanPendapatanResellerExcel"
    />
  </div>
</template>