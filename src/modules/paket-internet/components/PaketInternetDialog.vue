...
watch(paket, (nilai) => {
  if (nilai && props.open) isiForm(nilai)
})

// Reset form setiap kali dialog dibuka agar tidak ada sisa data paket sebelumnya
watch(
  () => [props.open, props.paketId] as const,
  ([open]) => {
    if (!open) return
    resetForm()
    if (modeEdit.value && paket.value) isiForm(paket.value)
  },
)
...
<template>
  <Dialog :open="open" @update:open="handleClose">
    <DialogContent class="sm:max-w-lg">
      ...
      <form v-else class="max-h-[70vh] space-y-4 overflow-y-auto pr-1" novalidate @submit.prevent="onSubmit">
        <div class="space-y-2">
