...
const filterFields: FilterFieldConfig[] = [
  {
    key: 'peran',
    label: 'Peran',
    options: Object.entries(peranAdminEnum).map(([value, meta]) => ({ value, label: meta.label })),
  },
  {
    key: 'status_aktif',
    label: 'Status',
...
const columns: ColumnDef<AdminLengkap, unknown>[] = [
  ...
  { accessorKey: 'email', header: 'Email' },
  {
    accessorKey: 'peran',
    header: 'Peran',
    cell: ({ row }) => h(StatusBadge, { value: row.original.peran, map: peranAdminEnum }),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) =>
      h('div', { class: 'flex justify-end gap-2' }, [
        h(
          Button,
          { as: 'RouterLink', to: `/admin/super-admin/admin/${row.original.id}/ubah`, variant: 'outline', size: 'sm' },
          () => 'Ubah',
        ),
        row.original.status_aktif
          ? h(
              Button,
              {
                variant: 'destructive',
                size: 'sm',
                onClick: () => (adminDikonfirmasi.value = row.original),
              },
              () => 'Nonaktifkan',
            )
          : null,
...
<template>
  <div class="space-y-4">
    ...
    <ConfirmDialog
      :open="!!adminDikonfirmasi"
      judul="Nonaktifkan admin ini?"
      :deskripsi="`${adminDikonfirmasi?.nama_lengkap} tidak akan bisa login lagi, tapi data & riwayatnya tetap tersimpan.`"
      label-konfirmasi="Nonaktifkan"
      variant-konfirmasi="destructive"
      :loading="isPendingNonaktifkan"
      @update:open="(v) => !v && (adminDikonfirmasi = null)"
      @confirm="handleNonaktifkan"
    />
  </div>
</template>
