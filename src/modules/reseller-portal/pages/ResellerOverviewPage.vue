...
import { Contact, UserCheck, Wifi, AlertTriangle } from 'lucide-vue-next'
  ...
  <div class="space-y-6 px-2">
    <DashboardSection title="Ringkasan Pelanggan" description="Data pelanggan milik reseller Anda.">
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <DashboardStatCard :icon="Contact" label="Total Pelanggan" :value="data?.stats?.total_pelanggan ?? 0" :loading="isLoading" />
        <DashboardStatCard :icon="Wifi" label="Layanan Aktif" :value="data?.stats?.pelanggan_aktif ?? 0" :loading="isLoading" />
    ...
    <Card>
      ...
      <CardContent class="pb-3">
        ...
        <div v-else-if="data?.pelanggan_terbaru?.length" class="divide-y">
          <RouterLink
            v-for="pelanggan in data.pelanggan_terbaru"
            :key="pelanggan.id"
          ...
          >
            ...
            <span class="shrink-0 text-xs text-muted-foreground">
              {{ pelanggan.layanan_internet?.[0]?.paket_internet?.nama_paket ?? pelanggan.layanan_internet?.[0]?.nama_paket_custom ?? 'Belum ada layanan' }}
            </span>
          </RouterLink>
        </div>
