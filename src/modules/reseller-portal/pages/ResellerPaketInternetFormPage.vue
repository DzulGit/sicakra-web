...
const { data: paket, isLoading: isLoadingDetail } =
  useResellerPaketInternetDetail(id)
...
const { mutate: simpan, isPending: isPendingSimpan } =
  useSimpanResellerPaketInternet()

const { mutate: ubah, isPending: isPendingUbah } =
  useUbahResellerPaketInternet()

const isPending = computed(
  () => isPendingSimpan.value || isPendingUbah.value,
)
    ...
    ubah(
      ...
      },
      {
        onSuccess: () => {
          ...
          router.push('/reseller/paket-internet')
        },
        onError: (error) => {
      ...
      >
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="space-y-2">
            ...
            <p
              v-if="errors.kecepatan_mbps"
              class="text-xs text-destructive"
            >
              {{ errors.kecepatan_mbps }}
            </p>
          </div>
          ...
          <div class="space-y-2">
            ...
            <p
              v-if="errors.jumlah_perangkat"
              class="text-xs text-destructive"
            >
              {{ errors.jumlah_perangkat }}
            </p>
          </div>
        </div>

        <div class="space-y-2">
          <Label for="promo_gratis_bulan">
            Promo Gratis (bulan)
          </Label>
          ...
          <p
            v-if="errors.promo_gratis_bulan"
            class="text-xs text-destructive"
          >
            {{ errors.promo_gratis_bulan }}
          </p>

          <p class="text-xs text-muted-foreground">
            Pelanggan baru dengan paket ini otomatis bebas
            tagihan pada pemasangan pertama.
          </p>
        </div>
        ...
        >
          <Checkbox
            ...
            @update:model-value="
              (v) => setFieldValue('status_aktif', !!v)
            "
          />
