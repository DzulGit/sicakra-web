...
const {
  ...
  isLoading,
  refetch,
} = useResellerTagihanDetail(id)

const {
  mutate: bayarTunai,
  isPending: isBayarTunaiPending,
} = useResellerBayarTunaiTagihan()

const {
  mutate: perbaruiLink,
  isPending: isPerbaruiLinkPending,
} = useResellerPerbaruiLinkTagihan()

const showTunaiDialog = ref(false)
const jumlahBulanTunai = ref('1')

const opsiBulan = Array.from(
  { length: 12 },
  (_, i) => i + 1,
)
...
const totalTunai = computed(() => {
  ...
  return (
    Number(tagihan.value.harga_snapshot) *
    Number(jumlahBulanTunai.value)
  )
})
...
const layanan = computed(
  () => tagihan.value?.layanan_internet,
)
...
function handlePerbaruiLink() {
  perbaruiLink(id.value, {
    onSuccess: () => {
      toast.success(
        'Link pembayaran berhasil diperbarui.',
      )
...
function bukaDialogTunai() {
  jumlahBulanTunai.value = String(
    tagihan.value?.jumlah_bulan ?? 1,
  )
  ...
  bayarTunai(
    ...
    },
    {
      onSuccess: () => {
        ...
        toast.success(
          'Pembayaran tunai berhasil dicatat. Tagihan lunas.',
        )
...
function waHref(nomor: string) {
  ...
  const intl = bersih.startsWith('0')
    ? '62' + bersih.slice(1)
    : bersih
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="isLoading">
      <Skeleton class="h-96 w-full max-w-2xl" />
    </div>
      ...
      >
        ...
        <Card v-if="pelanggan">
          ...
          >
            ...
            <div
              v-if="pelanggan.nomor_pelanggan"
            ...
            <div v-if="pelanggan.nomor_hp">
              ...
              <a
                :href="waHref(pelanggan.nomor_hp)"
                target="_blank"
            ...
            <div
              v-if="layanan?.alamat_pemasangan"
            >
              <p class="text-muted-foreground">
                Alamat Instalasi
              </p>
              ...
              <p>
                {{
                  [
                    layanan.detail_alamat,
                    layanan.alamat_pemasangan,
                  ]
                    .filter(Boolean)
                    .join(', ')
                }}
            ...
            >
              ...
              <p>
                ...
                —
                {{
                  layanan.paket_internet.kecepatan_mbps
                }}
                Mbps
              </p>
            </div>
          ...
          >
            ...
            <div class="flex justify-between">
              ...
              <span>
                ...
                —
                {{
                  tagihan.kecepatan_snapshot_mbps
                }}
                Mbps
              </span>
            </div>
          ...
          >
            ...
            <div class="flex justify-between">
              ...
              <span>
                {{
                  formatTanggal(
                    tagihan.xendit_invoice_expires_at,
                  )
                }}
              </span>
            </div>
            ...
            <div class="flex justify-between">
              ...
              <span>
                {{
                  tagihan.xendit_invoice_retry_count ||
                  '0'
                }}/3
              </span>
            </div>
            ...
            >
              <RefreshCw
                v-if="isPerbaruiLinkPending"
                class="mr-2 size-3 animate-spin"
              />

              {{
                isPerbaruiLinkPending
                  ? 'Memperbarui...'
        ...
        >
          ...
          >
            <p class="text-muted-foreground">
              Pelanggan membayar tunai kepada
              reseller. Setelah pembayaran
              dicatat, tagihan menjadi lunas.
            </p>

            <Button
              class="w-full"
              variant="outline"
              @click="bukaDialogTunai"
            >
              <HandCoins
                class="mr-2 size-4"
              />
        ...
        <!-- Dialog Bayar Tunai -->
        <Dialog
          :open="showTunaiDialog"
          @update:open="
            (value) => (showTunaiDialog = value)
          "
        >
          <DialogContent
            class="sm:max-w-sm"
          >
            <DialogHeader>
              ...
              <DialogDescription>
                Pilih berapa bulan yang dibayar
                tunai. Tagihan langsung lunas
                dan masa aktif layanan bertambah.
              </DialogDescription>
            </DialogHeader>
            ...
            >
              <Button
                variant="outline"
                :disabled="isBayarTunaiPending"
                @click="
                  showTunaiDialog = false
                "
              >
                Batal
              </Button>
