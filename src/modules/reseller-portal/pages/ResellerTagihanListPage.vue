...
const columns = [
  col.accessor('nomor_tagihan', {
    header: 'Nomor',
  }),

  col.accessor(
    (row) => row.layanan_internet?.pelanggan?.nama_lengkap,
    {
      id: 'nama_pelanggan',
  ...
  col.accessor('xendit_invoice_status', {
    ...
    cell: ({ row }) => {
      const status = row.original.xendit_invoice_status
    ...
    cell: ({ getValue }) =>
      h(
        Button,
        {
          ...
          size: 'sm',
          onClick: () =>
            router.push(`/reseller/tagihan/${getValue()}`),
        },
        () => 'Detail',
      ),
  }),
...
<template>
  <div class="space-y-4">
    ...
    <DataTable
      :columns="columns as ColumnDef<TagihanRow, unknown>[]"
      :data="hasil?.data ?? []"
      :loading="isLoading"
